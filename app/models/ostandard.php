...
class oStandard extends oMySql {
    ...
    function showEditListItem($pShow = true, $pEdit = true, $pDelete = false) {
        ...
        if (!($this->dID->getValue() > 0)) {
            ...
            if ($pShow || $pEdit || $pDelete) {
                $ret .= '<td>';
                if ($pShow)
                    $ret .= $this->getShowLink();
                if ($pEdit)
                    $ret .= $this->getEditLink();
                if ($pDelete)
                    $ret .= $this->getDeleteLink();
                $ret .= '</td>';
            }
            $ret .= '</tr>';
        }
    ...
    function showEditListVarNames() {
        ...
        foreach ($var_names as $var_name) {
            if (is_object($this->$var_name) && (substr($var_name, 0, 1) == 'd') && ($var_name != 'dEncryptID') && ($var_name != 'dPassword') && (($var_name != 'dID') || (hSession::getAdminrole() == SUPER_ADMIN))
            ) {
